...
export default function Assessments() {
  ...
  const [query, setQuery] = React.useState<Query>({ search: "", module_id: "", type: "", status: "" })

  // Dialog state
  const [dialogOpen, setDialogOpen] = React.useState(false)
  const [editing, setEditing] = React.useState<UIAssessment | null>(null)
  const [initialForm, setInitialForm] = React.useState<Partial<AssessmentFormValues> | undefined>()
  ...
  React.useEffect(() => { fetchAssessments() }, [fetchAssessments])
  ...
  async function openEditByRow(a: UIAssessment) {
    try {
      ...
      setInitialForm({
        ...
        open_at: d.open_at ?? "",
        close_at: d.close_at ?? "",
        description: "", // if you have it in resource, map it here
      })
      setDialogOpen(true)
    } catch (e: any) {
  ...
  async function saveAssessment(values: AssessmentFormValues) {
    ...
    try {
      if (values.id) {
        await assessmentsApi.update(values.id, values)
        toast("Assessment updated.")
      } else {
        await assessmentsApi.create(values)
  ...
  const extraFilters: DataTableExtraFilter[] = [
    {
      key: "module_id",
      label: `Module: ${modules.find(m => String(m.id) === query.module_id)?.title ?? query.module_id}`,
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Create, schedule, and manage assessments.</p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select
          ...
          onValueChange={(v) => setQuery((q) => ({ ...q, module_id: v === "all" ? "" : v }))}
        >
        ...
        <Button variant="outline" onClick={fetchAssessments}>Apply</Button>
        <Button
          variant="ghost"
          onClick={() => { setQuery({ search: "", module_id: "", type: "", status: "" }); fetchAssessments() }}
        >
          Reset
        </Button>
      </div>
